...
void Kuwahara_float(float2 uv, float2 texelSize, float radius, float edgeSharpness, out float3 colour)
{
    ...
    for (int i = -regionHalfSize; i <= regionHalfSize; i++) {
        for (int y = -regionHalfSize; y <= regionHalfSize; y++) {
            ...
            float2 tlUV = uv + perpendicularDir * texelSize;
            float2 trUV = uv + perpendicularDir * texelSize;
            float2 blUV = uv + perpendicularDir * texelSize;
            float2 brUV = uv + perpendicularDir * texelSize;
            ...
            // float variance = ((diff.r*diff.r) + (diff.g*diff.g) + (diff.b*diff.b)) * 0.25;
